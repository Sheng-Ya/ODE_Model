...
def cardiovascular_system(t, state, params, heart_control_inputs, resp_control_inputs, updates, all_time, num_removed):
    ...
    # alpha ranges between 0 (corresponding to the beginning of contraction) and 1
    alpha = (t % Tc) / Tc

    if (Tc / T_im) >= alpha >= 0:
        psi = np.sin(np.pi * (T_im / Tc) * alpha)
    elif (Tc / T_im) <= alpha <= 1:
        psi = 0
    ...
    # dXi_dt = 1 / T
    # integral, _ = quad(lambda t_: dXi_dt, t0, t)
    ...
    Pmax_lv = phi * Emax_lv * (V_lv - Vu_lv) + (1 - phi) * P0_lv * (np.exp(KE_lv * V_lv) - 1)
    ...
    P_lv = Pmax_lv - (KR_lv * Pmax_lv) * Q_lv + P_thor
    ...
    Pmax_rv = phi * Emax_rv * (V_rv - Vu_rv) + (1 - phi) * P0_rv * (np.exp(KE_rv * V_rv) - 1)
    ...
    P_rv = Pmax_rv - (KR_rv * Pmax_rv) * Q_rv + P_thor
    ...
    if VT_amv >= Vu_amv:
        ...
        V_amv = 0
        # P_amv = P_0 + P_im * (1 - (VT_amv / Vu_amv) ** -1.5)
        P_amv = P_0 + P_im
    ...
    if t != 0:
        if t < all_time[-1]:
            for key in [
                ...
                "Qi_lv", "Qi_rv", "phi", "S", "V_pv", "V_pp", "V_pa",  "P_amv"
            ]:
