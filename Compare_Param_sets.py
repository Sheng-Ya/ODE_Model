def compare_parameters(params1, params2, name1="Set 1", name2="Set 2"):
    keys_all = set(params1.keys()).union(params2.keys())

    print(f"\nComparing parameter sets: {name1} vs {name2}")
    print("-" * 60)
    ...
    for key in sorted(keys_all):
        ...
        if val1 is None:
            print(f"{key}: MISSING in {name1}, value in {name2} = {val2}")
            continue
        if val2 is None:
            print(f"{key}: value in {name1} = {val1}, MISSING in {name2}")
            continue

        # Case 2: both present → compare
        if val1 != val2:
            diff = val2 - val1
...
compare_parameters(Parameters, Parameters2, "Original", "Updated")
