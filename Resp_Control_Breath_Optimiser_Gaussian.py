...
def calculate_V_dV_dt(times, initial_guess, VA, VD, tolerance, E_rs, R_rs, P_ao):
    ...
    f_t1 = (a1 / E_rs) * t1 + (a2 / E_rs) * t1 ** 2 - ((2 * a2 * R_rs) / E_rs ** 2) * t1
    f_dash_t1 = a1 / E_rs + (2 * a2 / E_rs) * t1  - 2 * a2 * R_rs / (E_rs ** 2)

    k = Vt1 / (f_t1 + (f_dash_t1 / B) * (1 - np.exp(B * t1)))

    c1 = (k * f_dash_t1 * np.exp(B * t1)) / B

    d1 = k * (a1 * R_rs / (E_rs ** 2)) - k * (2 * a2 * (R_rs ** 2) / (E_rs ** 3)) - c1

    # Calculate for 0 <= times <= t1
    V[:split_idx] = (k * (a1 / E_rs) * x - k * (a1 * R_rs / (E_rs ** 2)) +
                    k * (a2 / E_rs) * (x ** 2) - k * (2 * a2 * R_rs / (E_rs ** 2)) * x +
                    k * (2 * a2 * (R_rs ** 2) / (E_rs ** 3)) +
                    c1 * np.exp(-B * x) + d1)
...
def objective(initial_guess, required_params, VAflow, VD, dt, tolerance):
    ...
    volume_signal, k = calculate_V_dV_dt(times, initial_guess, VAflow, VD, tolerance, E_rs, R_rs, P_ao)
    # P_musc *= k
    # dP_musc_dt *= k
    ...
    dV2_dt2_values_squared = ((1 / R_rs) * (dP_musc_dt -
                                                      E_rs * dV_dt_values)) ** 2
...
def calculate_single_V_dV_dt(t, initial_guess, VA, VD, tolerance, E_rs, R_rs, P_ao):
    ...
    if t <= t1:
        constant = (B * np.exp(-B * t1)) / (a1 / E_rs + (2 * a2 / E_rs) * t1 - 2 * a2 * R_rs / (E_rs ** 2))
        c1 = Vt1 / (constant * t1 * (a1 / E_rs + (a2 / E_rs * t1) - (2 * a2 * R_rs) / (E_rs ** 2)) + np.exp(-B * t1) - 1)
        k = c1 * constant
        # c1 = (Vt1 - k * ((a1 / E_rs) * t1 + (a2 / E_rs) * (t1 ** 2) - (2 * a2 * R_rs / (E_rs ** 2)) * t1)) / (
        #         np.exp(-B * t1) - 1)
        d1 = k * (a1 * R_rs / (E_rs ** 2)) - k * (2 * a2 * (R_rs ** 2) / (E_rs ** 3)) - c1

        V = (k * (a1 / E_rs) * t - k * (a1 * R_rs / (E_rs ** 2)) +
             k * (a2 / E_rs) * (t ** 2) - k * (2 * a2 * R_rs / (E_rs ** 2)) * t +
             k * (2 * a2 * (R_rs ** 2) / (E_rs ** 3)) +
             c1 * expBz + d1)
        dV_dt = k * (1 / R_rs) * (a1 * t + a2 * (t ** 2) - E_rs * V)
    else:
        mu = t1 + 0.5 * B * tau
        term1 = - (t1 * t1) / tau
        term2 = (mu ** 2) / tau
