...
# X3 = np.load('DGSM_filtered_LHCS_500000_X_sample_HR_Plv_Prv_Vlv_Vrv_rest.npy')[100000:(100000+66000),:]
# X4 = np.load('DGSM_filtered_LHCS_500000_X_sample_HR_Plv_Prv_Vlv_Vrv_rest.npy')[300000:(300000+121200),:]# size [N, 299]
# Result1 = np.load('Result_DGSM_chunked_0_8400.npy')
# Result2 = np.load('Result_DGSM_chunked_8400_32400.npy')
# Result3 = np.load('Result_DGSM_chunked_100000_66000.npy')
# Result4 = np.load('Result_DGSM_chunked_300000_121200.npy')
...
p_sys_mask = Result[:, 1] < 250
X = X[p_sys_mask, :]
Result = Result[p_sys_mask]

p_dia_mask = Result[:, 2] < 150
X = X[p_dia_mask, :]
Result = Result[p_dia_mask]

EDV_mask = (Result[:, 3] < 250) & (Result[:, 3] > 100)
X = X[EDV_mask, :]
Result = Result[EDV_mask]

ESV_mask = Result[:, 4] < 150
X = X[ESV_mask, :]
Result = Result[ESV_mask]

RV_V_max_mask = (Result[:, 5] < 300) & (Result[:, 5] > 100)
X = X[RV_V_max_mask, :]
Result = Result[RV_V_max_mask]

RV_V_min_mask = Result[:, 6] < 150
X = X[RV_V_min_mask, :]
Result = Result[RV_V_min_mask]

RV_P_max_mask = Result[:, 7] < 60
X = X[RV_P_max_mask, :]
...
SV_mask = Result[:, 9] < 100
X = X[SV_mask, :]
Result = Result[SV_mask]

eject_mask = Result[:, 10] < 80
X = X[eject_mask, :]
