...
GaussianProcess_final = joblib.load("GaussianProcessMatern32_10000_rest_no_p_thor.joblib")
# ----------------------------
# OBSERVATION
# ----------------------------
# change
...
"ESV": (50, 10), "Max RV Volume": (186, 21), "Min RV Volume": (52, 9), "Max RV Pressure": (25, 2),
"Min RV Pressure": (2, 1), "Min RA Volume": (36, 5), "Max RA Volume": (93, 16), "Min RA Pressure A descent": (2, 1),
...
"Max LA Pressure Mitral Opening": (7, 2), "Min LA Pressure V descent": (2, 1), "LA EDV": (46, 5), "RA EDV": (47, 5),
"LV Pressure Deriv": (1600, 150), "RV Pressure Deriv": (1200, 150), "Tidal Volume": (0.5, 0.1),
"Minute Ventilation": (6.5, 0.5), "Cardiac Output": (85, 4.2), "PaO2": (95, 4.5), "PaCO2": (40, 2), "Percentage Volume Change": (0.2, 0.1)}
...
if __name__ == "__main__":
    ...
    # mask = np.ptp(X_all, axis=0) != 0  # ptp = max - min, 0 means all values identical
    # X = X_all[:, mask]
    ...
    # X[0] = Parameters
    # param_values = torch.tensor(X[0], dtype=torch.float32)
    # Result_tensor, Var_tensor = GaussianProcess_final.model.predict_mean_and_variance(param_values)
    ...
    # plt.show()
    size = 10000
    _ = hmw.run_waves(n_waves=1, n_simulations=2, n_test_samples=size, refit_on_all_data=False, refit_emulator_on_last_wave=False)
    ...
    np.save (f"NROY_Points_rest.npy", nroy_points)
    np.save(f"NROY_Params_rest.npy", params_post_hm)
