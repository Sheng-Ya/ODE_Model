...
class HistoryMatchingWorkflow(HistoryMatching):
    ...
    def _load_rest_overlap_reference(self):
        ...
        missing_params = [name for name in self.overlap_params if name not in subset_vars]
        if missing_params:
            raise ValueError(
                "Rest posterior is missing overlap/rest parameters: "
                + ", ".join(missing_params)
            )
    ...
    def _plot_parameter_ranges(self, full_samples: TensorLike, all_param_names: list[str]) -> None:
        ...
        Blue   = rest posterior/reference parameters
    ...
    def pre_wave_train_emulators(self, n_simulations: int = 4096, refit_on_all_data: bool = False) -> None:
        ...
        if os.path.exists(x_train_path) and os.path.exists(y_train_path):
            ...
            # Generate hybrid samples (clouded rest posterior/reference plus exercise-only uniform)
    ...
    def run(
        ...
        if phys_seed_only:
            ...
            # Wave 0 starts from clouded rest-posterior/reference candidates.
            # Simulations are then clouded around the physiological seed set.
    ...
    def run_waves(
        ...
        use_phys_seed_wave0: bool = False,
        ...
        use_phys_seed_wave0: bool
            If True, keep the older wave-0 physiological seeding shortcut. If False,
            wave 0 uses the normal NROY loop, whose first proposal cloud-samples the
            rest posterior/reference block and uniformly samples the exercise-only block.
