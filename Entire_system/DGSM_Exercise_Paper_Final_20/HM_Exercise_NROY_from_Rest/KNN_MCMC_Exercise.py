...
Expects in HM run directory:
  ...
  Emulator_exercise_wave/{output_name}/GaussianProcessMatern32_{output_name}_best.joblib
Older *_constant.npy files and Emulator_exercise_only_wave/ are still accepted.

Usage:
  python3 KNN_MCMC_Exercise.py
  python3 KNN_MCMC_Exercise.py --hm-run-dir /path/to/run_dir
...
_parser.add_argument(
    "--hm-run-dir",
    default=None,
    help="History-matching run directory produced by Bayesian_Calibration_Exercise.py.",
)
_parser.add_argument(
    "--threads-per-chain",
    type=int,
...
HM_RUNS_ROOT = SCRIPT_DIR


def _resolve_project_path(path):
    """Resolve relative paths from this script directory, matching the HM script."""
...
def _first_existing_path(description, candidates, predicate=os.path.exists):
...
def _resolve_emulator_dir():
    requested = os.environ.get("HM_EMULATOR_DIR") or os.environ.get("EMULATOR_DIR")
    if requested:
        requested_path = requested
        if not os.path.isabs(requested_path):
            requested_path = os.path.join(root, requested_path)
        return _first_existing_path(
            "exercise emulator directory",
            [requested_path],
            os.path.isdir,
        )
    ...
    return _first_existing_path(
        "exercise emulator directory",
        [
            os.path.join(root, "Emulator_exercise_wave"),
            os.path.join(root, "Emulator_exercise_only_wave"),
        ],
        os.path.isdir,
    )
...
EMULATOR_DIR = _resolve_emulator_dir()     # GP emulators from last refitted wave
out_dir = os.path.join(root, "MCMC_start_50_Exercise_Only")
os.makedirs(out_dir, exist_ok=True)
print(f"Using HM run directory: {root}", flush=True)
print(f"Using emulator directory: {EMULATOR_DIR}", flush=True)
...
output_names = [
    ...
    "Max_LA_Pressure_Mitral_Opening", "LA_Pre_Atrial_Contraction_Volume",
    "RA_Pre_Atrial_Contraction_Volume", "LV_Pressure_Deriv",
    "RV_Pressure_Deriv", "Tidal_Volume", "Minute_Ventilation",
    "PaO2", "PaCO2",
]

# ================================================================
# EXERCISE TARGETS: {name_with_spaces: (population_mean, population_variance)}
# ================================================================
...
"LA Pre Atrial Contraction Volume": (0.25, 0.0025), "RA Pre Atrial Contraction Volume": (0.25, 0.0025),
...
LA_PRE_DISPLAY_MEAN, LA_PRE_DISPLAY_STD = _propagated_vpre_display_stats(
    ...
    observation["LA Pre Atrial Contraction Volume"][0],
    observation["LA Pre Atrial Contraction Volume"][1],
)
RA_PRE_DISPLAY_MEAN, RA_PRE_DISPLAY_STD = _propagated_vpre_display_stats(
    ...
    observation["RA Pre Atrial Contraction Volume"][0],
    observation["RA Pre Atrial Contraction Volume"][1],
)

# ================================================================
# CALIBRATION PARAMETER SUBSET (from DGSM sensitivity analysis) # change
# ================================================================
subset_vars_set = {'alpha2', 'beta2', 'C_amv', 'C_bv', 'C_ev', 'C_O2_param3', 'C_pa', 'C_pp', 'C_pv', 'C_rmv', 'C_sa',
               'f_ac_max', 'f_acCO2_n', 'fes_max', 'G_ap', 'GEmax_rv', 'gM', 'GR_amp', 'GV_amv', 'GV_dead', 'GV_ev',
               'Io_sh', 'K1_vc', 'k_ac', 'KcCO2', 'KcMRV', 'kev', 'MO2_ampn', 'MO2_bp', 'P_n_max', 'PaO2_ac_n',
               'phi_max', 'R_amp0', 'R_bpn', 'R_pa', 'R_tr', 'scale_param3', 'tauMR', 'theta_po_max', 'theta_spn',
               'theta_v', 'VA_rest', 'Vu_hv', 'Vu_pp', 'Vu_pv', 'Vu_vc', 'Wc_v', 'Wp_v', 'Ysv_max', 'Yv_max'}
...
nroy_points_path = _first_existing_path(
    "NROY points file",
    [
        os.path.join(root, f"NROY_Points_exercise_{PERCENT}.npy"),
        os.path.join(root, f"NROY_Points_exercise_{PERCENT}_constant.npy"),
    ],
    os.path.isfile,
)
nroy_params_path = _first_existing_path(
    "NROY params file",
    [
        os.path.join(root, f"NROY_Params_exercise_{PERCENT}.npy"),
        os.path.join(root, f"NROY_Params_exercise_{PERCENT}_constant.npy"),
    ],
    os.path.isfile,
)
print(f"  NROY points: {nroy_points_path}", flush=True)
print(f"  NROY params: {nroy_params_path}", flush=True)

nroy_points_np = np.load(nroy_points_path)
nroy_params_dict = np.load(nroy_params_path, allow_pickle=True).item()
