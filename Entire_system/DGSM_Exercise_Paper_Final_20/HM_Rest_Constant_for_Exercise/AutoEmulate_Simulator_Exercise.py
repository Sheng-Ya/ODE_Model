...
BUFFER_LIMIT = 80000
EXERCISE_STIM_TIME = 60.0
INITIAL_EXERCISE_DURATION = 180.0
CONVERGENCE_CHUNK_TIME = 60.0
MAX_CONVERGENCE_EXTENSIONS = 5
HR_CONVERGENCE_TOL = 0.03
max_time = EXERCISE_STIM_TIME + INITIAL_EXERCISE_DURATION
...
class Cardiopulmonary(Simulator):
    ...
    def _latest_simulation_time(self, storage):
        latest_nonzero_index = (int(storage["i"].item()) - 1) % BUFFER_LIMIT
        return float(storage["all_time"][latest_nonzero_index])

    def _hr_range_in_buffer(self, storage):
        i_buffer = int(storage["i"].item()) % BUFFER_LIMIT
        hr = np.concatenate((storage["HR_store"][i_buffer:], storage["HR_store"][:i_buffer]))
        hr = hr[np.isfinite(hr)]
        if hr.size == 0:
            return float("inf")
        return float(np.max(hr) - np.min(hr))
    ...
    def simulate_cpu(self, Current_Parameters, local_updates, old_parameters):
        ...
        ODE_solution = None
        t_span = [0.0, max_time]
        for attempt in range(MAX_CONVERGENCE_EXTENSIONS + 1):
            ...
            hr_range = self._hr_range_in_buffer(local_updates)
            latest_time = self._latest_simulation_time(local_updates)
            if hr_range < HR_CONVERGENCE_TOL:
                break
            if attempt == MAX_CONVERGENCE_EXTENSIONS:
                print(f"Not converged: {hr_range}, {latest_time}")
                break

            IC_current = ODE_solution.y[:, -1]
            if latest_time <= t_span[0]:
                latest_time = float(ODE_solution.t[-1])
            t_span = [latest_time, latest_time + CONVERGENCE_CHUNK_TIME]
    ...
    def safe_simulate_cpu(self, params, storage, old_parameters, timeout=600):
        try:
            signal.alarm(0)
