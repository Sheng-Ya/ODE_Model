## Build an emulator for each target by varying only parameters that are
# sensitive for that target (not the union)
...
def parallel_simulations(param_samples, storage, n_jobs, chunk_size=500, save_path_results='Result_chunked.npy', save_path_states='States_chunked.npy'):
    results_all = []
    final_states = []
...
if __name__ == "__main__":
    ...
    cols_to_drop = [9, 11, 14, 15, 17, 20, 21, 22, 27, 30]

    print("Kept outputs:", output_names_full)

    # =========================
    # 3) Loop over targets (kept outputs) and train one emulator each
    # =========================
    params = {
        "epochs": 200,
        "lr": 0.1,
    ...
    for j, target_name in enumerate(output_names_full):
        ...
        np.save(f"{out_dir}/{target_name}_LHCS_X.npy", X)
        ...
        np.save(f"{out_dir}/{target_name}_LHCS_Result_raw.npy", Result)
        ...
        # ---- drop constant X columns (store mask for inference) ----
        feature_mask = np.ptp(X, axis=0) != 0
        X = X[:, feature_mask]

        # ---- cap training size ----
        size = min(TRAIN_SIZE_CAP, Result_all.shape[0])
        print(f"Samples after filtering for {target_name}: {Result_all.shape[0]} -> using {size}")
        X = X[:size, :]
        ...
        # ---- train ----
        ae = AutoEmulate(X, Y, log_level="info", models=[MODEL_NAME], model_params=params)
        ae.summarise()
        best = ae.best_result()
        ...
        # ---- save ----
        joblib.dump(best, f"{out_dir}/{MODEL_NAME}_{target_name}_best.joblib")
        joblib.dump(
            {"feature_mask": feature_mask, "cols_to_drop": cols_to_drop, "output_names_kept": output_names_full},
            f"{out_dir}/{MODEL_NAME}_{target_name}_meta.joblib"
        )
        ae.plot(best, fname=f"{out_dir}/{MODEL_NAME}_{target_name}.png")


    # # Heart Rate
    # os.makedirs(f"Heart_Rate", exist_ok=True)
    #
    # subset_vars = {'T0', 'GT_v', 'P_n', 'Vu_sv0', 'fev_o', 'fab_o', 'fev_inf', 'V_nominal', 'Wb_sh', 'C_O2_param1',
    #                'f_ab_max', 'GT_s', 'Vu_ev0'}
    # filtered_names = []
    # filtered_bounds = []
    # for name, bound in zip(sp["names"], sp["bounds"]):
    #     filtered_names.append(name)
    ...
    # param_keys = list(sp_filtered["names"])
    #
    # # # Run Simulator to obtain samples
    # X = sample_inputs_from_spec(sp_filtered, n_samples=500, random_seed=42, method="lhs")
    # X = X.cpu().numpy() if X.is_cuda else X.numpy()
    # np.save('Heart_Rate/Heart_rate_LHCS_X_rest_no_Pthor_Vtot_22_01_2026.npy', X)
    # param_samples = [dict(zip(param_keys, row)) for row in X]
    # Result = parallel_simulations(param_samples, Next_Conditions, n_jobs=250)
    # Result = np.asarray(Result)
    # np.save('Heart_Rate/Heart_rate_LHCS_Result_rest_no_Pthor_22_01_2026.npy', Result)
    #
    # # # Train emulator
    # mask = Result[:, 0] != 0
    # X = X[mask, :]
    # Result_all = Result[mask]
    #
    # # Min RA Volume, Min RA Pressure A descent, Min RA Pressure V descent, Min LA Volume, Min LA Pressure A descent,
    # # Min LA Pressure V descent, LA ESV, RA ESV, Cardiac Output
    # cols_to_drop = [9, 11, 14, 15, 17, 20, 21, 22, 27]
    # Result_all = np.delete(Result_all, cols_to_drop, axis=1)
    #
    ...
    # X = X[:, mask]
    #
    # size = min(1000, Result.shape[0])
    # print(f"Number of samples used to train the emulator is {size}")
    # X = X[:size, :]
    # Result = Result[:size]
    #
    # Result_heart_rate = Result[:, 0]
    #
    # params = {
    #     "epochs": 200,
    #     "lr": 0.1,
    ...
    #
    # print("\n" + "=" * 90)
    # print(f"Training emulator for target Heart Rate")
    # print("=" * 90)
    #
    # ae = AutoEmulate(X, Result_heart_rate, log_level="info", models=["GaussianProcessMatern32"], model_params=params)
    #
    # ae.summarise()
    # best = ae.best_result()
    #
    ...
    # print(f"Target: Heart_Rate | R² test: {r2_test} | RMSE test: {rmse_test}")
    # print(f"Best model id: {best.id} | model_name: {best.model_name}")
    # print(best.params)
    #
    # # save model per target
    # joblib.dump(best, "Heart_Rate/Heart_Rate_rest_no_pthor_vtot_target.joblib")
