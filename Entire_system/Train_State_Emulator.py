...
# Unwanted states constant values: {"theta_change_O2_sp": 0.0, "theta_change_O2_sv": 0.0, "P_n_current": 92.0, "MRTO2": 0.00243333333, "MRTCO2": 0.00324166667, "MRV": 0.0, "VE_integral": 0.0}
Unwanted_states = {"theta_change_O2_sp", "theta_change_O2_sv", "P_n_current", "MRTO2", "MRTCO2", "MRV", "VE_integral"}
constant_values = {"theta_change_O2_sp": 0.0, "theta_change_O2_sv": 0.0, "P_n_current": 92.0, "MRTO2": 0.00243333333, "MRTCO2": 0.00324166667, "MRV": 0.0, "VE_integral": 0.0}
...
for j, state_name in enumerate(kept_states):
    ...
    print("\n" + "=" * 80)
    print(f"Training emulator for state [{j}]: {state_name}")
    print("=" * 80)

    # single-output target (N, 1)
    Y_state = Result_states[:, j:j+1]
    ...
    print(
        f"State: {state_name} | "
        f"R² test: {best.r2_test}"
    )
    # save model with state name
    joblib.dump(best, f"best_{model_name}/{model_name}_{state_name}_{size}_rest_no_p_thor_state.joblib")