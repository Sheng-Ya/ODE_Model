...
class Cardiopulmonary(Simulator):
    ...
    def simulate_cpu(
        ...
        P_peri_cycle_max_idx = np.array(
            [b0 + np.argmax(P_peri[b0:b1]) for b0, b1 in zip(P_peri_cycle_idx[:-1], P_peri_cycle_idx[1:])])
        ...
        result = torch.tensor([np.mean(past_10_flat_segments), np.mean(P_sa[P_sa_max_idx]), np.mean(P_sa[open_idx1]),
            ...
            cardiac_output, Pa_O2, Pa_CO2, Vol_percentage_change, mean_max_P_peri], dtype=torch.float32).unsqueeze(0)
