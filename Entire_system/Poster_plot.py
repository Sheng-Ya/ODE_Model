...
# ---------- Load EXERCISE data ----------
X_ex = np.load('All_params_DGSM_500_X_samples_HR_P_sys_P_dia_atria.npy')
Result_ex = np.load('All_params_DGSM_500_Results_HR_Plv_Prv_Vlv_Vrv_atria.npy')
HR_ex = Result_ex[:, 2]
...
X_rest = np.vstack((X1, X2))
Result_rest = np.vstack((Result1, Result2))
HR_rest = Result_rest[:, 2]
...
Si_ex = dgsm.analyze(sp, X_ex, HR_ex, print_to_console=False)
Si_rest = dgsm.analyze(sp, X_rest, HR_rest, print_to_console=False)

# Convert to numpy arrays
dgsm_ex, names_ex, conf_ex = np.array(Si_ex['dgsm']), np.array(Si_ex['names']), np.array(Si_ex['dgsm_conf'])
dgsm_rest, names_rest, conf_rest = np.array(Si_rest['dgsm']), np.array(Si_rest['names']), np.array(Si_rest['dgsm_conf'])

# ---------- Get top 10 for REST ----------
top_idx_rest = np.argsort(dgsm_rest)[::-1][:5]
top_names_rest = names_rest[top_idx_rest]
top_dgsm_rest = dgsm_rest[top_idx_rest]
top_conf_rest = conf_rest[top_idx_rest]

# ---------- Get top 10 for EXERCISE ----------
top_idx_ex = np.argsort(dgsm_ex)[::-1][:5]
top_names_ex = names_ex[top_idx_ex]
top_dgsm_ex_raw = dgsm_ex[top_idx_ex]
top_conf_ex_raw = conf_ex[top_idx_ex]

# ---------- Build x-axis order ----------
# Start with rest top 10
all_top_names = list(top_names_rest)

# Append exercise-only top 10 that are not in rest's list
for n in top_names_ex:
    if n not in all_top_names:
        all_top_names.append(n)

# Build aligned values
rest_vals = [dgsm_rest[names_rest.tolist().index(n)] if n in names_rest else 0 for n in all_top_names]
rest_conf = [conf_rest[names_rest.tolist().index(n)] if n in names_rest else 0 for n in all_top_names]
ex_vals = [dgsm_ex[names_ex.tolist().index(n)] if n in names_ex else 0 for n in all_top_names]
ex_conf = [conf_ex[names_ex.tolist().index(n)] if n in names_ex else 0 for n in all_top_names]
...
plt.bar(x - width/2, rest_vals, width, yerr=rest_conf, label='Rest', alpha=0.8, capsize=2)
plt.bar(x + width/2, ex_vals, width, yerr=ex_conf, label='Exercise', alpha=0.8, capsize=2)

plt.xticks(x, all_top_names, rotation=90)
# HR
# f_v_b0 - minimum vagal firing rate given minimal baroreceptor input
# custom_labels = ["Baseline HR", "Tot Blood Vol", "f$_{v,B0}$", "HR Vag Gain", "B$_{min}$ Set point",
#                  "HR Sym Gain", "Inspired O$_{2}$ %"]
# P_sa
# custom_labels = ["Tot Blood Vol", r"$pCO_2 \rightarrow cCO_2$ Param", "V$_{u,sv0}$", "pCO2$_{art,n}$",
#                  "B$_{min}$ Set point", "B$_{max}$ Set point"]

# ESV
# custom_labels = ["Tot Blood Vol", "E$_{max,lv0}$", "V$_{u,sv0}$", r"$pO_2 \rightarrow cO_2$ Param",
#                  r"$\tau_\mathrm{fall}$", "LV E Gain", "V$_{u,lv}$"]

# plt.xticks(x, custom_labels, fontsize=15, fontweight='bold')

plt.ylabel("Sensitivity Index (DGSM)")
plt.title("Min RV Pressure")
# plt.title("Systolic Blood Pressure")
# plt.title("End Systolic Volume")
