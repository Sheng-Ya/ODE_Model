...
sp = ProblemSpec({
        ...
        'names': [
            ...
            "P0_ra", "P0_lv", "P0_rv", "g_abd", "g_thor", "P_abdmax_n", "P_abdmin_n", "P_thormax_n", "P_thormin_n",
            "VT_n", "A_im", "Tc", "T_im", "s",
            # cardio control
        ...
        'bounds': [
            ...
            [0.2 * lower, 0.2 * upper], [0 * lower, 0.1 * upper], [-0.3997 * upper, -0.3997 * lower],
            ...
            [1 * lower, 1 * upper], [1.5 * lower, 1.5 * upper], [0 * lower, 0.1 * upper],
...
.evaluate(rbf_final_loaded.predict)
.analyze_sobol()
 )
...
total.to_csv("sobol_total.csv")
first.to_csv("sobol_first.csv")
second.to_csv("sobol_second.csv")

# Sort total-order indices (ST) from highest to lowest
ranked = total.sort_values("ST", ascending=False)

# Create plot
fig, ax = plt.subplots(figsize=(6, 12))  # adjust height if you have many parameters

# Reorder the DataFrame for plotting in ranked order
ranked_plot = barplot(ranked, ax=ax)

# Log scale and title
ax.set_yscale("log")
ax.set_title("Sobol Total-Order Sensitivities (Ranked)")

# Annotate each bar with rank
for i, (name, value) in enumerate(zip(ranked.index, ranked["ST"])):
    ax.text(value, i, f"#{i+1}", va="center", ha="left", fontsize=10, color="blue")
...
sp.heatmap("HR", "ST", "Total Order Sensitivity", ax1)
sp.heatmap("HR", "S1", "First Order Sensitivity", ax2)
plt.show()
...
sp.heatmap("HR", "ST", "Total Order", ax=ax1)
sp.heatmap("HR", "ST_conf", "Total Order Conf.", ax=ax2)
sp.heatmap("HR", "S1", "First Order", ax=ax3)
sp.heatmap("HR", "S1_conf", "First Order Conf.", ax=ax4)
plt.show()