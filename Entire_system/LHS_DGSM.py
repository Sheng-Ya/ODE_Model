...
# Result = (sp.finite_diff.sample(5)
# .evaluate(rbf_final_loaded.predict)
#  )

# X = finite_diff.sample(sp, 1000)
# Result = rbf_final_loaded.predict(X)
X = np.load('DGSM_1000_X_samples_HR.npy')


n_jobs = -1  # use all CPUs
chunk_size = 1000  # or use len(X) // n_jobs
chunks = [X[i:i + chunk_size] for i in range(0, len(X), chunk_size)]

# Run in parallel
with tqdm_joblib.tqdm_joblib(tqdm(total=len(chunks), desc="Predicting chunks")):
    Result_chunks = Parallel(n_jobs=n_jobs)(
        delayed(rbf_final_loaded.predict)(chunk) for chunk in chunks
    )
