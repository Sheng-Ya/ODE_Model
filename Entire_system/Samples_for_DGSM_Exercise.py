...
if __name__ == "__main__":
    ...
    # ---- Load MAP from KNN_MCMC_Rest run -----------------------------
    # Each calibrated parameter's bounds get re-centred on its MAP value
    # (bounds become [MAP * lower, MAP * upper] instead of
    #  [nominal * lower, nominal * upper]). Non-calibrated parameters
    # keep their original bounds, including any hardcoded ones.
    MAP_RUN_DIR = "MCMC_Rest_20_21_04_3000_logspline_copula_prior"
    _map_sample  = np.load(os.path.join(MAP_RUN_DIR, "map_sample.npy"))
    _subset_vars = np.load(os.path.join(MAP_RUN_DIR, "subset_vars.npy"),
                           allow_pickle=True).tolist()
    assert len(_map_sample) == len(_subset_vars), (
        f"map_sample length {len(_map_sample)} != subset_vars length "
        f"{len(_subset_vars)}"
    )
    map_values = {str(name): float(val)
                  for name, val in zip(_subset_vars, _map_sample)}
    print(f"Loaded MAP for {len(map_values)} calibrated params from "
          f"{MAP_RUN_DIR}/")
    ...
    np.save("DGSM_500_X_exercise_20_21_04.npy", X)
    # X = np.load("DGSM_500_X_rest_20_21_04.npy")
    ...
    np.save(f'DGSM_Result_exercise_20_21_04.npy', Result)