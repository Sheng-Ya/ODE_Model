Initial_Conditions = {
    # Table 1. Systemic arteries

# cardiovascular system = {
# state
    "VT_pa": 0,
    "VT_pp": 116.6775,
    "VT_pv": 114,
    "Q_pa": 4,
    "VT_la": 90,
    "VT_lv": 130,
    "VT_ra": 90,
    "VT_rv": 130,
    "VT_sv": 1500,
    "VT_bv": 370,
    "VT_hv": 120,
    "VT_rmv": 300,
    "VT_amv": 500,
    "VT_ev": 700,
    "P_sp": 5,
    "V_sa": 22,
    "P_sa": 77,
    "Q_sa": 7,
    "VT_vc": 350,
# initial condition of inputs from other controllers
    "U": 0,
    "beta": 0,
    "BF": 0.33,
    "Vu_ev": 607.8,
    "Vu_amv": 286.4,
    "Vu_rmv": 190.95,
    "Vu_sv": 1361.6,
    "R_ep": 1.655,
    "R_amp": 3.51,
    "R_rmp": 5.27,
    "R_sp": 2.49,
    "R_bp": 6.57,
    "R_hp": 19.71,
    "HR": 1.7,
    "Emax_lv": 2.392,
    "Emax_rv": 1.412,
    "I": 0,

# cardiovascular controller
    "theta_change_O2_sp": 0,
    "theta_change_CO2_sp": 0,
    "theta_change_O2_sv": 0,
    "theta_change_CO2_sv": 0,
    "theta_change_O2_sh": 0,
    "theta_change_CO2_sh": 0,
    "P_tilda": 92,
    "f_ac": 8.0807,
    "f_ap": 4.4492,
    "R_ep_change": 0,
    "R_sp_change": 0,
    "R_rmp_n_change": 0,
    "R_amp_n_change": 0,
    "Vu_ev_change": 0,
    "Vu_sv_change": 0,
    "Vu_rmv_change": 0,
    "Vu_amv_change": 0,
    "Emax_lv_change": 0,
    "Emax_rv_change": 0,
    "Ts_change": 0,
    "Tv_change": 0,
    "xb_O2": 0,
    "xb_CO2": 0,
    "xh_O2": 0,
    "xh_CO2": 0,
    "Wh": 0,
    "xrm_O2": 0,
    "xrm_CO2": 0,
    "xam_O2": 0,
    "xM": 0,
    "x_met": 0,
# initial condition of inputs from other controllers
    "Wh_lv": 0,
    "Wh_rv": 0,
    "Ca_O2": 20,
    # "Q_bp": 0,
    "Q_hp": 0.5,
    "Q_rmp": 2.17,
    "Q_amp": 15.4,
    # "times": [0],
    # "f_sp_history": [],
    # "f_sh_history": [],
    # "f_v_history": [],
    # "phi_met_history": [],
    "VT": 0.5,
    "dP_sa_dt": 0,
    # "PaO2": 80,
    # "PaCO2": 40,
    # "MRTCO2":
    "TI": 0.8,
    # "previous_VE": [],

# Respiratory mechanics
    "V": 0,
    "alpha": 0,
    "Vflow_ua": 0,
# other inputs
#     "Nd": [0, 0, 0, 0, 0, 0],

# Respiratory controller
    "VE_integral": 0,
# other inputs
    "dV_dt": 0.1,
    # "previous_dV_dt": [0],
    "P_musc": 0,
    "WI": 0,
    "WE": 0,
    # "previous_WI": [0],
    # "previous_WE": [0],
    # "Pa_CO2": 40,
    # "Pa_O2": 80,
    "PbCO2": 48.5383,
    # "MRV": 0,

# Gas exchange
    "Pd_1_O2": 104.3637,
    "Pd_1_CO2": 39.5616,
    "Pd_2_O2": 104.3637,
    "Pd_2_CO2": 39.6736,
    "Pd_3_O2": 104.0505,
    "Pd_3_CO2": 39.8127,
    "Pd_4_O2": 103.8005,
    "Pd_4_CO2": 40.0061,
    "Pd_5_O2": 103.3579,
    "Pd_5_CO2": 40.3359,
    "Pa_O2": 45, # ignore: set 0 here, but it is another value initially from Gas_Exchange.py
    "Pa_CO2": 40, # ignore: set 0 here, but it is another value initially from Gas_Exchange.py
    "dPa_O2_dt": 0.3557,
    "dPa_CO2_dt": -0.2465,
    "PA_O2": 102.5153,
    "PA_CO2": 40.9432,
    "PvbCO2": 0, # set 0 here, but it is another value initially from Gas_Exchange.py
    "PCSFCO2": 40,
    "MRTO2": 0.33,
    "MRTCO2": 0.3,
    "CvO2": 0.1639,
    "CvCO2": 0.5247,
    "MRV": 0,
    # other inputs from other systems
    # "dV_dt":
    # "V": 0,
    "Q_pp": 35.5,
    "Q_bp": 1.5, # or 10.5
    "Q_la": 150,
    "VD": 0.15,
}