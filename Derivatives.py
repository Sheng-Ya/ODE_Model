...
#             # No wrap-around
#             delay_index = np.searchsorted(all_time[:heart_index + 1], delay_time, side='right') - 1
#         else:
#             # Wrap-around
#             idx_in_sorted = np.searchsorted(all_time[heart_index + 1:], delay_time, side='right') - 1
...
def model_derivatives(t, state, params, heart_inputs, heart_control_inputs, gas_exchange_inputs, resp_control_inputs, updates, num_removed, t_start, time_saved, i, all_time, BUFFER_LIMIT):
    ...
    if t - time_since_beat > T:
        if time_since_beat >= all_time[0]: # No wrap-around
            idx_in_2 = np.searchsorted(all_time[:heart_index + 1], time_since_beat, side='right') - 1
        else:  # Wrap-around
            idx_in_sorted2 = np.searchsorted(all_time[heart_index + 1:], time_since_beat, side='right') - 1
