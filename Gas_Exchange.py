...
def gas_exchange(t, state, params, all_time, resp_control_inputs, heart_system_inputs, updates, num_removed, t_start, previous_Selected_Conditions, time_saved, i, BUFFER_LIMIT):
    ...
    (a2, alpha1, alpha2, beta1, beta2, C2, Fi_CO2, Fi_O2, K1, K2, PACO2_Delay_IC, PAO2_Delay_IC, P_atm,
     ...
     VTCO2, VTO2, MRCO2, MRO2, tau_MRV, s, Ta) = \
        (params[k] if k in params else Parameters[k] for k in [
            ...
            "MO2_bp", "VB", "MRTCO2_basal", "MRTO2_basal", "tauMR", "VTCO2", "VTO2", "MRCO2", "MRO2", "tau_MRV", "s",
            "Ta"
        ])
    ...
    dV_dt = resp_control_inputs["dV_dt_store"][resp_control_index]
    Q_pp = heart_system_inputs["Q_pp_store"][heart_index]/1000
