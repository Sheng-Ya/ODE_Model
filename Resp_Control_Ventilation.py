...
def resp_control_vent(t, state, params, updates, gas_exchange_inputs, num_removed, t_start, time_saved, i, BUFFER_LIMIT):
    ...
    if t <= (t1 + t2) and updates["finish_breath_time"] == 0:
        ...
        if resp_cycle < updates["resp_cycle_store"][(i - num_removed - 1) % BUFFER_LIMIT] and (updates["resp_cycle_store"][(i - num_removed - 1) % BUFFER_LIMIT] - resp_cycle) > 1:
        ...
        if t != t_start:
            if resp_cycle < updates["resp_cycle_store"][(i - num_removed - 1) % BUFFER_LIMIT] and (updates["resp_cycle_store"][(i - num_removed - 1) % BUFFER_LIMIT] - resp_cycle) > 1: # restarts
