...
# A = np.load("IC_final_resp.npy", allow_pickle=False)
#
# state_names = [
...
BUFFER_LIMIT = 50000
Next_Conditions = np.load("Next_final_resp.npy",  allow_pickle=True).item()

i = Next_Conditions["i"].item() % BUFFER_LIMIT

last_idx = (i - 1) % BUFFER_LIMIT

for key, value in Next_Conditions.items():
    if isinstance(value, np.ndarray):
        # scalar-like arrays (e.g. i, j)
        if value.ndim == 1 and value.size == 1:
            print(f"{key}: {value.item()}")
        # circular buffers
        elif value.ndim == 1 and value.size == BUFFER_LIMIT:
            print(f"{key}: {value[last_idx]}")